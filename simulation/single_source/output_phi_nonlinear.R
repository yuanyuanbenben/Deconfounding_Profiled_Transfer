...
load("varying_phi_single_source_nonlinear.Rdata")
...
p_beta_L1 <- ggplot(df_beta_L1, aes(x = phi, y = value, color = method)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Beta L1 Loss", x = "phi", y = "Loss") +
  theme_minimal()

# 2. Plot for Beta L2 Loss (x-axis is p)
p_beta_L2 <- ggplot(df_beta_L2, aes(x = phi, y = value, color = method)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Beta L2 Loss", x = "phi", y = "Loss") +
  theme_minimal()

# 3. Plot for Eta L1 Loss (x-axis is p)
p_eta_L1 <- ggplot(df_eta_L1, aes(x = phi, y = value, color = method)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Eta L1 Loss", x = "phi", y = "Loss") +
  theme_minimal()

# 4. Plot for Eta L2 Loss (x-axis is p)
p_eta_L2 <- ggplot(df_eta_L2, aes(x = phi, y = value, color = method)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Eta L2 Loss", x = "phi", y = "Loss") +
  theme_minimal()
...
write_xlsx(df_results, "output/single_nonlinear_phi.xlsx")
