...
source("estimate_function_nonlinear.R")
...
for (phi_cur in phi_list) {
  ...
  for (repeat_index in 1:repeat_times) {
    ...
    # parameters
    beta_s = matrix(rep(c(1,0),times = c(s,p-s)),p,1,byrow = TRUE)
    eta = matrix(rep(c(-2,0,1),times = c(s_delta,p-2*s_delta,s_delta)),p,1,byrow = TRUE)
    beta_t = beta_s + eta
...
save(df_results, file = "varying_phi_single_source_nonlinear.Rdata")
