source("utils.R")
source("estimate_function_ini.R")
...
for (p_cur in p_list) {
  ...
  for (k in seq_len(nrow(map))) {
    ...
    raw_errors_long <- rbind(
      raw_errors_long,
      data.frame(
        p    = p_cur,
        rep    = seq_len(repeat_times),
        method = method_k,
        metric = metric_k,
        value  = perrep[[key_k]]
      )
...
#df_summary2 <- compare_methods_summary(df_results)
#View(df_summary2)

save(df_results, file = "varying_p_multi_source_linear_with_sd_ini2.Rdata")
