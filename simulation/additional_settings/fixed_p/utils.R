...
Q_alter_trim <- function(X,rho=0.5){
  UDV_list = svd(X)
  U = UDV_list$u
  D = UDV_list$d
  V = t(UDV_list$v)
  tau = quantile(D, rho)
  Dtilde = pmin(D, tau)
  Q = diag(ncol(X)) - t(V) %*% diag(1-(Dtilde/D)**2) %*% V
  return(Q)
}
...
# oracle estimation for H and E
find_HE <- function(X,Psi,n=300,q=3,max_iter=1000,tor=1e-3){
  H_working = matrix(0,n,q,byrow = TRUE)
  for (iter in 1:max_iter) {
    grad = ((4/(1+exp(-H_working%*%Psi)) + 4*sin(H_working%*%Psi) - X)*(4*exp(-H_working%*%Psi)/(1+exp(-H_working%*%Psi))**2 + 4*cos(H_working%*%Psi)))%*%t(Psi)
    H_working_ = H_working - 0.05*grad
    if (iter%%10==0){
      print(iter)
      if (sum(H_working_ - H_working)**2 /n/q < tor){
        E_working = X - 4/(1+exp(-H_working_%*%Psi)) - 4*sin(H_working_%*%Psi)
        return(list('H'=H_working_,'E'=E_working))
      }
    }
    H_working  = H_working_
  }
  E_working = X - 4/(1+exp(-H_working_%*%Psi)) - 4*sin(H_working_%*%Psi)
  return(list('H'=H_working_,'E'=E_working))
}
