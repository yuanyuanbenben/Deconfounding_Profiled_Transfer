rm(list = ls())
source("utils.R")
#source("update_estimation_function.R")
...
for (p_cur in p_list) {
  ...
  for (repeat_index in 1:repeat_times) {
    ...
    result = with_preserve_seed(
      transfer_estimation(X_s, y_s, X_t, y_t, 
                          true_beta_s = beta_s_true,
                          trim_form = trim_form)
    )
    eta_hat    <- as.vector(result$eta)
    beta_t_hat <- as.vector(result$beta_t)
...
save(df_results, file = "varying_p_single_source_linear_with_sd_ini2.Rdata")
