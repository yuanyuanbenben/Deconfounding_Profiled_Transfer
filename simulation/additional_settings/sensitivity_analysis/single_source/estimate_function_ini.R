...
library(withr)
# our method
transfer_estimation <- function(X_s,y_s,X_t,y_t,rho=0.5,
                                true_beta_s = NULL,
                                trim_form = 'standard',linear=TRUE){
  n_s = dim(X_s)[1]
  n_t = dim(X_t)[1]
  p = dim(X_s)[2]
  X = rbind(X_s,X_t)
  ...
  #     intercept = FALSE
  #   )
  #   as.numeric(coef(ridge_fit, s = ridge_fit$lambda.1se)[-1])
  # })
