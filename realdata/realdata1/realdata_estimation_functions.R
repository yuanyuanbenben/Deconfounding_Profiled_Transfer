...
Q_trim <- function(X,rho=0.5,trim_form='standard',quant=30){
  UDV_list = svd(X)
  U = UDV_list$u
  D = UDV_list$d
  V = t(UDV_list$v)
...
transfer_estimation_selection <- function(X_s,y_s,X_t,y_t,rho=0.5,trim_form = 'standard',linear=TRUE){
  n_s = dim(X_s)[1]
  n_t = dim(X_t)[1]
  p = dim(X_s)[2]
  X = rbind(X_s,X_t)
  ...
  # eta = Lasso_func(QX_t, Qy_t-Z_t-QX_t%*%beta_initial)$beta
  
  # v^{(k)} 
  eta1 = Lasso_func(QX_t,Qy_t-QX_t%*%beta_initial-Z_t)$beta
  v_value = max(abs(t(QX_t)%*%(Qy_t-Z_t-QX_t%*%beta_initial-QX_t%*%eta1)))/n_t/log(p)
  print(v_value)
  #estimate eta by y_t-Z_t-X_t*beta_initial~X_t
  ret_list = list('v_value'=v_value)
}
